# CV Roman Shuliatiev – Product Designer

## Product Designer with 5+ years of experience | Ukraine, Kyiv

| - | - |

| Seeking a full-time Product designer role in a product-focused companyto apply my expertise in designing and delivering valuable products.Proven track record of designing intuitive UX for web and mobile platformsand launching successful apps with over 600K installs.I have a diverse range of experience, including software development,product design, and managing a team of 3 designers. | [linkedin.com/in/shuliatievroman](http://linkedin.com/in/shuliatievroman)

[roman.novodvorskiy@gmail.com](mailto:roman.novodvorskiy@gmail.com)

[t.me/artificially_busy](http://t.me/artificially_busy)

# WORK & RELEVANT EXPERIENCE

## Product Designer @ Yesim

 | 2021 – Present 4+ years full-time, remote | Project details: Worldwide eSIM store, Marketplace

Job duties and responsibilities:

- ● Design intuitive and functional interfaces for web and mobile products acrossboth B2B and B2C contexts
- ● Create and develop (with AI assistant) interactive prototypes
- ● Conducting competitor and UX research
- ● Create and maintain design systems
- ● Regular audit of the company's product line to improve UI/UX
- ● Provide clear and effective UX copy to support user goals
- ● Create promo materials for marketing activities
- ● Continuously iterate based on feedback and performance insights
- ● Generating and validating hypotheses, launching A/B tests
- ● Conducting moderated usability testing
- ● UI quality assurance activities

## Product Designer @ [SMBF.me](http://SMBF.me)

Project details: Online reputation platform, SaaS  | 2020 – 20211 year 9 monthsfull-time, remote |

Job duties and responsibilities:

- ● Collaboration with product and development teams
- ● Develop and design user-centered concepts, flows, wireframes, and prototypes
- ● Write, edit, and improve clear and effective UX copy to support user goals
- ● Enhance existing web and mobile platforms through UX improvements

## Android Developer* / Product Designer @ Eventssion

Project details: Event management and online ticketing platform

Job duties and responsibilities:

- ● Participated in the development of two Android apps using Java and related tools*
- ● Collaboration with Product and Development Teams
- ● Managed a group of 3 designers
- ● Design concepts, user flows, wireframes and prototypes
- ● Conduct usability studies with end-users
- ● Ensure a consistent user experience across multiple platforms
- shifted my role to a Product Designer through a career progression within the project. | 2016 – 20204 years 3 monthsfull-time, office |

### | ## Information Technology Specialist @ Japan Tobacco International

Job duties and responsibilities:

- ● Gather requirements from the business analysis team
- ● Develop and maintain company's internal software tools

# PET PROJECTS

## | ## Score Counter, Android App

Project details: Most loved Android companion for board games sessions. 600K total installs. 5.0⋆ rating.

 | Jan 2016 – Present |

My role

- ● I built this app as a side project
- ● Full-cycle development and all related activities
- ● Conceptualization and ideation of project ideas

I was proud to achieve the following results:

- ● Ranked #1 in Google Play search results
- ● 600K installs / 50,000 MAU
- ● 5.0 rating

## | ## Paragraphus, Android App

Project details: Paragraphus offers a refreshing way to discover world literature basedpurely on content. I built this app as a side project during the COVID-19 Pandemic.  | 2020 |

My role

- ● Conceptualization and ideation of project ideas
- ● Full-cycle development and all related activities

# SKILLS & TOOLS

Design Skills: Product Design, User Experience, Mobile App Design, Design Systems, Prototyping, A/B Testing,UX Writing, Stakeholder Alignment • UX Strategy • Design Thinking • Project Planning • User-Centered Design • Wireframing • Journey Maps • Interaction Design • User Research • User Interviews • Prototyping • Usability Testing • Visual Design • Accessibility • UI design • Design Systems • Developer Handoffs • Cross-Functional Collaboration • Figma • Adobe CC • Web/Native, Figma, Adobe Tools, HTML/CSS, SQL/BigQuery, GA4, Looker Studio

# EDUCATION

| Specialist Degree in Computer Science @ Kremenchuk Mykhailo Ostrohradskyi National University

Languages

Ukrainian, Russian, English